...
# toolchain specific
ENVIR_ARM_GCC= ./enviroment/gcc-arm-none-eabi-8-2018-q4-major/bin/
BOARD=/media/pid4rv/FRDM-KL46Z
TOOLCHAIN = ${ENVIR_ARM_GCC}arm-none-eabi-
CC = $(TOOLCHAIN)gcc
CXX = $(TOOLCHAIN)g++
...
TARGET = frdmkl46z
TARGET_EXT = elf
LD_SCRIPT = ./sys_lib/$(FOLDER_NAME)/$(LINKER_NAME).ld
...
# directories
INC_DIRS = ./sys_lib ./sys_lib/$(FOLDER_NAME) .
# app headers directories (remove comment and add more files)
INC_DIRS += ./header

SRC_DIRS = ./sys_lib ./sys_lib/$(FOLDER_NAME) .
...
all: create_outputdir $(OBJ_FOLDER)$(TARGET).$(TARGET_EXT) print_info run
...
print_info:
	...
	#${TOOLCHAIN}objcopy -O srec $(OBJ_FOLDER)$(TARGET).$(TARGET_EXT) $(OBJ_FOLDER)$(TARGET).srec
	@echo ' '
run:
	@cp ./build/frdmkl46z.bin ${BOARD}
	@echo '-------------------------RUN COMPLETE------------------------'
